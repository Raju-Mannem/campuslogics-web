import { getCategoryTypePosts } from '@/services/post.services';
...
export default async function CategoryPage({
  ...
  try {
    const result = await getCategoryTypePosts({
      published: true,
      page: currentPage,
      limit: limit,
    ...
    <section className="min-h-screen">
      <div className="relative border-b border-gray-200 overflow-hidden px-8 sm:px-24 py-12">
        <h1 className="text-white text-3xl font-bold mb-8 sm:mb-12 bg-brand-500/20 p-2 rounded-lg">
          {(categoryType ?? '').charAt(0).toUpperCase() + categoryType?.slice(1)} Notifications
        </h1>
